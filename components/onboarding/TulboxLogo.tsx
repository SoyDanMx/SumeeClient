/**
 * Componente del Logo de TulBox
 * 
 * El logo de TulBox consiste en:
 * - Icono: Apretón de manos dentro de un corazón (púrpura/morado)
 * - Texto: "TulBox" (S y M mayúsculas, resto minúsculas)
 * 
...
const LOGO_COLORS = {
    light: {
        icon: '#820AD1', // Púrpura TulBox oficial (RGB: 130, 10, 209)
        text: '#1F2937', // Gris oscuro
    },
    dark: {
        icon: '#A855F7', // Púrpura claro TulBox
...
export function TulBoxLogo({
    ...
    const USE_LOGO_IMAGES = true; // ✅ Usa icon.png (S sobre púrpura) - mismo que la app
    
    let logoSource: any = null;
    let useImage = false;
    
    // Usar icon.png (S blanca sobre púrpura) - consistente con TulBoxClient
    if (USE_LOGO_IMAGES) {
        ...
        >
            <View style={[
                styles.iconContainer,
                {
                    ...
                    backgroundColor: useImage ? '#FFFFFF' : undefined,
                    borderRadius: dimensions.icon * 0.2,
                    overflow: 'hidden',
                },
            ]}>
                {useImage ? (
                    // icon.png - S blanca sobre púrpura (mismo que la app)
                    <Image
                        source={logoSource}
                        style={[
                            styles.logoImage,
                            { width: dimensions.icon, height: dimensions.icon, backgroundColor: '#FFFFFF' },
                        ]}
                        resizeMode="contain"
                    />
                ) : (
