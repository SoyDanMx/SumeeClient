-- ============================================================================
-- FASE 1.2: CREAR TABLAS - VERSIÓN CORREGIDA
-- ============================================================================
-- CORRECCIÓN: Error "column discipline does not exist" en función find_similar_services
-- ============================================================================
...
-- ============================================================================
-- 2.5: Función find_similar_services (VERSIÓN ROBUSTA)
-- ============================================================================
-- CORRECCIÓN: No depende de service_catalog.discipline
-- Solo usa service_embeddings que tiene su propia columna discipline
-- ============================================================================
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(sc.min_price, 0)::NUMERIC AS min_price  -- Usar COALESCE por si no existe
    FROM public.service_embeddings se
    LEFT JOIN public.service_catalog sc ON sc.id = se.service_id  -- LEFT JOIN para ser más robusto
    WHERE 
        (discipline_filter IS NULL OR se.discipline = discipline_filter)
        AND (sc.is_active = true OR sc.is_active IS NULL)  -- Permitir NULL por si no existe
